 ...
 *
 */
public class HHT_104_02  extends BusinessService implements ProcessUnit {
	...
	public String process(Request request) {
		...
		log.info("operation hht104.02店店调拨--调拨商品检索, response="+bean.getResponse().toString());
	...
	private void processData(BusinessBean bean) throws Exception{
		...
		System.out.println("HHT_104_02------------〉计时开始时间："
				+ DateUtils
						.DateFormatToString(DateUtils
								.getSystemDate(),
								"yyyy-MM-dd HH:mm:ss.SSS") + "    "
				+ System.nanoTime());
		List<Map> list=jdbcTemplateUtil.searchForList(sqlpropertyUtil.getValue(bean.getUser().getStore(),"hht104.02"),new Object[]{store, sku, sku, sku});
		System.out.println("HHT_104_02------------〉计时结束时间："
				+ DateUtils
						.DateFormatToString(DateUtils
								.getSystemDate(),
								"yyyy-MM-dd HH:mm:ss.SSS") + "    "
