...
public class HHT_001_03  extends BusinessService implements ProcessUnit {
...
//		log.info("response=" + bean.getResponse().toString());
		...
		log.info("operation hht001.03, response="+bean.getResponse().toString());
	...
	private void processData(BusinessBean bean) throws Exception {
			List<Map> list=jdbcTemplateUtil.searchForList(sqlpropertyUtil.getValue(bean.getUser().getStore(),"hht001.03"),new Object[]{bean.getRequest().getParameter(configpropertyUtil.getValue("bc"))});
			...
			if(list.size()<1){
				...
				if(s.trim().equals("2")){
					bean.getResponse().setCode(BusinessService.errorcode);
					bean.getResponse().setDesc("该箱状态为"+PullDownUtils.getHHTStatusList().get(s)+"，不可无该箱");
				}else if(s.trim().equals("1") && !crtusr.equals(bean.getUser().getName())){
					bean.getResponse().setCode(BusinessService.errorcode);
					...
					List<Map> data=jdbcTemplateUtil.searchForList(sqlpropertyUtil.getValue(bean.getUser().getStore(),"hht001.03.04"),new Object[]{bean.getRequest().getParameter(configpropertyUtil.getValue("bc"))});
...
//					}
					...
					sql[0]=sqlpropertyUtil.getValue(bean.getUser().getStore(),"hht001.03.01");
					o[0]=new Object[]{sysId,bean.getRequest().getParameter(configpropertyUtil.getValue("bc"))};
					int i=1;
					for(Map tmp:data){
						sql[i]=sqlpropertyUtil.getValue(bean.getUser().getStore(),"hht001.03.02");
						o[i]=new Object[]{sysId,bean.getRequest().getParameter(configpropertyUtil.getValue("bc")),tmp.get("HHTSKU"),tmp.get("HHTPQT"),hhtflc,hhttlc };
				//				bean.getUser().getAttribute("HHTFLC"), bean.getUser().getAttribute("HHTTLC")};
						i++;
					}
					sql[sqlSize-1]=sqlpropertyUtil.getValue(bean.getUser().getStore(),"hht001.03.03");
					String username="";
//					String username = bean.getUser() != null?bean.getUser().getName():"s1777";
					...
					o[sqlSize-1]=new Object[]{sysId,username,bean.getRequest().getParameter(configpropertyUtil.getValue("bc"))};
