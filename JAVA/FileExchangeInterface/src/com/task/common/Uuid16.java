...
public class Uuid16 implements Serializable{
	...
	private static Logger logger = Logger.getLogger( Uuid16.class );
    ...
    public static Uuid16 read(DataInput in)
        throws IOException
    {
...
//            System.out.println("total=" + (end - begin) + "ms," + (end - begin) / 1000L + " second");
           ...
           int failcnt =0;
        	for(int i=0;i<100;i++){
                ...
                String s2 =  java.util.UUID.randomUUID().toString().split("-")[0];
                if(s2.length()<=8){
                	cnt++;
                }else{
                	failcnt++;
        	...
        	System.out.println("cnt="+cnt+"   failcnt="+failcnt);
        }
